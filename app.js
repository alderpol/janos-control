...
const ADDONS = [
  ...
  ["libro", "Libro Combo"], ["maqui", "Maquillaje"], ["drone", "Drone"], ["vipExtras", "Extras VIP"]
];
...
const BASE_RATES = {
  ...
  assistant: 18000, extraSheet: 6000, churchUpgrade: 60000
};
...
function createTasks(client) {
  ...
  if (client.addons.includes("libro")) definitions.push(task("partyBook", "Diseñar y pedir libro de fiesta", "Entrega", true, "COMPLEMENTOS", "Libro Fiesta (Fotografia Digital)", "partyBookDesign"));
...
function renderClients() {
  document.getElementById("clientsView").innerHTML = `<div class="toolbar"><div class="search"><input id="clientSearch" placeholder="Buscar por nombre, código o salón"></div><select id="clientPackFilter"><option value="">Todos los packs</option><option value="silver">Silver</option><option value="gold">Golden / All Inclusive</option><option value="vip">VIP</option><option value="informal">Informal</option></select><button class="secondary-btn" id="downloadClientTemplate">Plantilla CSV</button><button class="primary-btn" id="importClientsBtn">Importar lote</button><input id="clientCsvInput" type="file" accept=".csv,text/csv" hidden></div><div id="clientGrid" class="client-grid">${clientCards(state.clients)}</div>`;
}
function clientCards(clients) { return clients.length ? clients.sort((a,b)=>parseDate(a.eventDate)-parseDate(b.eventDate)).map(c => `<article class="client-card"><div class="client-card-top"><span class="tag">${packLabel(c.pack)}</span><span class="muted">${dateText(c.eventDate)}</span></div><h3>${escapeHtml(c.honoree)}</h3><p>#${escapeHtml(c.code)} · ${escapeHtml(c.salon)} · ${escapeHtml(c.type)}</p><div class="progress-line"><i style="width:${progress(c)}%"></i></div><div class="card-meta"><span>${progress(c)}% completo</span><span>${c.tasks.filter(t=>t.status==="pending").length} pendientes</span></div><div class="card-actions"><button class="secondary-btn" data-open-client="${c.id}">Ver tareas</button><button class="ghost-btn" data-edit-client="${c.id}">Editar</button></div></article>`).join("") : empty("No hay clientes", "Usá “Nuevo cliente” para comenzar."); }
...
function renditionRows(rows) { return rows.length ? rows.map(r=>{const c=state.clients.find(x=>x.id===r.clientId); return `<div class="rendition-row"><div><strong>${escapeHtml(r.work)}</strong><small>${escapeHtml(c?.honoree||"Cliente eliminado")}</small></div><span>${c?dateText(c.eventDate):"-"}</span><span class="muted">${escapeHtml(r.category)}</span><span class="money">${money(r.amount)}</span><select data-rendition-status="${r.id}">${Object.entries(RENDITION_STATUS).map(([k,v])=>`<option value="${k}" ${r.status===k?"selected":""}>${v}</option>`).join("")}</select></div>`;}).join("") : empty("Sin rendiciones", "Al completar un trabajo remunerado aparecerá aquí."); }
...
function parseAddons(raw){const text=String(raw||"").toUpperCase(),items=[];const rules=[["pant",/PANT/],["pixel",/PIXEL/],["miniflex",/UP\.?MFLEX|MINI\s*FLEX/],["flex",/UP\.FLEX|\bFLEX\b/],["libro",/LIBRO/],["maqui",/MAQUI/],["drone",/DRONE/]];rules.forEach(([key,regex])=>{if(regex.test(text))items.push(key);});if(items.includes("miniflex"))return [...new Set(items.filter(x=>x!=="flex"))];return [...new Set(items)];}
...
document.addEventListener("change", e => {
  ...
  if(e.target.id==="clientPackFilter")filterClients();
...
function filterClients(){const q=(document.getElementById("clientSearch")?.value||"").toLowerCase(),p=document.getElementById("clientPackFilter")?.value||"";const filtered=state.clients.filter(c=>(!p||c.pack===p)&&[c.honoree,c.clientName,c.code,c.salon].some(v=>String(v||"").toLowerCase().includes(q)));document.getElementById("clientGrid").innerHTML=clientCards(filtered);}
