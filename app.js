...
function attentionItems() {
  ...
  state.clients.forEach(c => {
    ...
    if(!c.isExternal&&hasPrintedBook&&!hasPhotoSession) items.push({days:-1,html:`<button class="attention-alert attention-conflict" data-open-client="${c.id}"><div class="attention-top"><strong>${escapeHtml(c.honoree)}</strong><span class="tag attention-salon-tag">${escapeHtml(c.salon||"")}</span></div><span>Conflicto de venta: Libro Combo sin sesión</span><small>El Libro Combo requiere una sesión de fotos. Agregá Mini Flex/Flex con "Sesión extra - fotografía", o Sans Souci, para habilitarla.</small></button>`});
