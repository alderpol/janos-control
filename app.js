...
function attentionItems() {
  ...
  state.clients.forEach(c => {
    ...
    if(!c.isExternal&&hasPrintedBook&&!c.tasks.some(t=>["bookCoveragePhoto","flexSessionPhoto"].includes(t.key))) items.push({days:-1,html:`<button class="attention-alert attention-conflict" data-open-client="${c.id}"><strong>${escapeHtml(c.honoree)}</strong><span>Conflicto de venta: Libro Combo sin sesión</span><small>El Libro Combo requiere una sesión de fotos. Agregá Mini Flex o Flex y elegí "Sesión extra - fotografía" para habilitarla.</small></button>`});
