...
function openPhotoSessionForm(clientId){const client=state.clients.find(item=>item.id===clientId);if(!client)return;const form=document.getElementById("photoSessionForm"),session=client.photoSession||{};form.reset();form.elements.clientId.value=client.id;form.elements.date.value=session.date||"";form.elements.time.value=session.time||"";const[sh,sm]=String(session.time||"").split(":");document.getElementById("sessionHour").value=sh||"";document.getElementById("sessionMinute").value=sm||"";form.elements.location.value=session.location||client.salon||"";form.elements.team.value=session.team||"";form.elements.includesFashionProduction.checked=Boolean(session.includesFashionProduction);form.elements.includesMakeupHair.checked=Boolean(session.includesMakeupHair);form.elements.notes.value=session.notes||"";document.getElementById("photoSessionDialog").showModal();}
function syncSessionTimeField(){const h=document.getElementById("sessionHour"),m=document.getElementById("sessionMinute"),form=document.getElementById("photoSessionForm");if(!h||!m||!form)return;form.elements.time.value=(h.value&&m.value)?`${h.value}:${m.value}`:"";}
document.getElementById("sessionHour")?.addEventListener("change",syncSessionTimeField);
document.getElementById("sessionMinute")?.addEventListener("change",syncSessionTimeField);
