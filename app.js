...
function whatsappNumber(value){let digits=String(value||"").replace(/\D/g,"").replace(/^00/,"");if(!digits)return "";if(digits.startsWith("549"))return digits;if(digits.startsWith("54"))return `549${digits.slice(2)}`;return `549${digits.replace(/^0/,"")}`;}
function whatsappMessage(client){const contact=client.clientName||client.honoree;const metadata=currentUser?.user_metadata||{};const sender=metadata.first_name||String(metadata.full_name||"").split(" ")[0];const intro=sender?`Soy ${sender}, del equipo de Fotografía y Video de Janos.`:"Te contacto desde el equipo de Fotografía y Video de Janos.";return `Hola ${contact}, ¿cómo estás? ${intro} Me comunico por el evento de ${client.honoree}, programado para el ${dateText(client.eventDate)} en ${client.salon}. Este será nuestro canal de contacto para coordinar todo lo relacionado con fotografía y video. ¡Un gusto acompañarte!`;}
