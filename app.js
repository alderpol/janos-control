...
function attentionItems() {
  ...
  state.clients.forEach(c => {
    ...
    if(bookDue){const when=days===0?"Evento hoy":`Faltan ${days} ${days===1?"día":"días"}`;items.push({days:sortDays,html:`<button class="attention-alert attention-blink" data-open-client="${c.id}"><strong>${escapeHtml(c.honoree)}</strong><span>Libro Combo / material impreso</span><small>${when} · Confirmar selección de fotos, diseño y envío a impresión.</small></button>`});}
