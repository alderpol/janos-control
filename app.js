...
const SESSION_SALONS = ["Pilar Hotel","Del Viso","Acceso Oeste","Hurlingham","Maschwitz","Benavidez 1","Benavidez 2","Villa de Mayo","Ituzaingó 2"];
...
let sessionPicker = { clientId: null, y: 0, m: 0, date: "", salon: "", time: "" };
...
function renderSessionSalons(){const{date,salon,clientId}=sessionPicker;const occupied=date?sessionsOnDate(date,clientId):[];const salonDelDia=occupied[0]?.salon||null;return SESSION_SALONS.map(n=>{const esSalonDelDia=salonDelDia===n,bloqueado=salonDelDia&&!esSalonDelDia,sel=salon===n;return `<div class="ssc${sel?" sel":""}${bloqueado?" bkd":""}" ${bloqueado?"":`data-session-salon="${n}"`}>${n}${esSalonDelDia?'<br><small style="color:var(--gold-2)">· ocupado ese día</small>':""}</div>`;}).join("");}
function renderSessionSlots(){const{date,salon,time,clientId}=sessionPicker;const occupied=(date&&salon)?sessionsOnDate(date,clientId).filter(s=>s.salon===salon).map(s=>s.time):[];return SESSION_SLOTS.map(s=>{const bkd=occupied.includes(s),sel=time===s;return `<div class="ssl${bkd?" bkd":""}${sel?" sel":""}" ${bkd?"":`data-session-slot="${s}"`}><div class="ssl-t">${s}</div><div class="ssl-s ${bkd?"bkd":sel?"sel":"ok"}">${bkd?"Ocupado":sel?"✓ Elegido":"Libre"}</div></div>`;}).join("");}
function renderSessionPicker(){document.getElementById("sessionCalendar").innerHTML=renderSessionCalendar();document.getElementById("sessionSalonGrid").innerHTML=sessionPicker.date?renderSessionSalons():`<p class="session-hint">Elegí un día primero.</p>`;document.getElementById("sessionSlotGrid").innerHTML=(sessionPicker.date&&sessionPicker.salon)?renderSessionSlots():`<p class="session-hint">Elegí un salón primero.</p>`;const form=document.getElementById("photoSessionForm");form.elements.date.value=sessionPicker.date;form.elements.location.value=sessionPicker.salon;form.elements.time.value=sessionPicker.time;const btn=document.getElementById("saveSessionBtn");if(btn)btn.disabled=!(sessionPicker.date&&sessionPicker.salon&&sessionPicker.time);}
function openPhotoSessionForm(clientId){const client=state.clients.find(item=>item.id===clientId);if(!client)return;const form=document.getElementById("photoSessionForm"),session=client.photoSession||{};form.reset();form.elements.clientId.value=client.id;form.elements.team.value=session.team||"";form.elements.includesFashionProduction.checked=Boolean(session.includesFashionProduction);form.elements.includesMakeupHair.checked=Boolean(session.includesMakeupHair);form.elements.notes.value=session.notes||"";const base=session.date?parseDate(session.date):new Date();sessionPicker={clientId:client.id,y:base.getFullYear(),m:base.getMonth(),date:session.date||"",salon:session.location||"",time:session.time||""};renderSessionPicker();document.getElementById("photoSessionDialog").showModal();}
...
function openClientForm(client=null) {
  ...
  if(client) ["code","eventDate","salon","type","honoree","clientName","clientPhone","whatsappGroupUrl","driveUrl","guests","pack","notes"].forEach(k=>form.elements[k].value=client[k]??"");
...
document.addEventListener("click", e => {
  ...
  const sessionDay=e.target.closest("[data-session-day]");if(sessionDay){const nd=sessionDay.dataset.sessionDay;if(sessionPicker.date!==nd){sessionPicker.salon="";sessionPicker.time="";}sessionPicker.date=nd;renderSessionPicker();return;}
  const sessionSalon=e.target.closest("[data-session-salon]");if(sessionSalon){if(sessionPicker.salon!==sessionSalon.dataset.sessionSalon)sessionPicker.time="";sessionPicker.salon=sessionSalon.dataset.sessionSalon;renderSessionPicker();return;}
  const sessionSlot=e.target.closest("[data-session-slot]");if(sessionSlot){sessionPicker.time=sessionSlot.dataset.sessionSlot;renderSessionPicker();return;}
