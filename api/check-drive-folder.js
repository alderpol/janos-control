...
// link: si la carpeta ya no esta, el frontend ofrece recrearla en vez de
// abrir un link roto.
//
// Es una sola llamada liviana (un GET a la API de Drive) por click del
// usuario en un cliente puntual - no hay chequeo automatico ni en
...
export async function handler(event) {
  ...
  try {
        ...
        const { data: client, error: clientError } = await supabase
            .from("clients")
            .select("id,owner_id,salon,drive_url")
            .eq("id", clientId)
            .maybeSingle();
        ...
        // Si el link no tiene forma de carpeta de Drive (por ej. fue pegado a
        // mano), no hay nada que verificar por API: asumimos que existe.
